...
const initialMessages: ChatMessage[] = [
  {
    id: '1',
    role: 'assistant',
    content: '¡Hola Vicente! 👋 Soy tu asistente de viajes de élite. Tu luna de miel de 45 días por 6 países va tomando forma. ¿En qué te puedo ayudar? Puedo sugerir restaurantes, organizar transfers o revisar tus reservas.',
      ...
      <div className="px-4 pb-4 pt-2 border-t border-border bg-background">
        <div className="flex items-center gap-2">
          <input
            ...
            className="flex-1 bg-secondary text-sm rounded-xl px-4 py-3 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all"
          />
          <button
            onClick={handleSend}
            disabled={!input.trim()}
            className="touch-target gradient-hero text-primary-foreground p-3 rounded-xl disabled:opacity-40 transition-opacity active:scale-95"
          >
...
function getMockResponse(input: string): string {
  ...
  if (lower.includes('hotel') || lower.includes('alojamiento')) {
    return '🏨 He revisado las opciones disponibles para esas fechas. Te sugiero buscar en la zona premium. ¿Quieres que lo agregue como borrador al itinerario? Recuerda: necesitarás subir la confirmación de reserva para validarlo.';
  }
  if (lower.includes('vuelo') || lower.includes('avión')) {
    return '✈️ Perfecto. He tomado nota del vuelo. Lo agregaré como borrador. Para confirmarlo, necesitaré que subas el e-ticket o confirmación de la aerolínea. ¿Necesitas que busque conexiones alternativas?';
  }
  return '📝 ¡Entendido! Lo agregaré al itinerario como borrador. Recuerda que puedes subir el comprobante de reserva para cambiar su estado a "Confirmado". ¿Algo más que necesites?';
}
